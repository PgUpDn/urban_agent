...
def _clean_trimesh(mesh: trimesh.Trimesh, min_height: float = 1e-6) -> None:
    """Remove degenerate faces using the modern trimesh API, with fallback."""
    mesh.remove_unreferenced_vertices()
    if hasattr(mesh, "nondegenerate_faces"):
        mask = mesh.nondegenerate_faces(height=min_height)
        mesh.update_faces(mask)
    else:  # pragma: no cover - legacy trimesh support
...
def _nearest_buildings(point_xy, building_metadata, top_k=2):
    px, py = point_xy
    distances = []
    for meta in building_metadata:
        cx, cy, _ = meta["centroid"]
        dist = math.hypot(px - cx, py - cy)
        distances.append((dist, meta["name"]))
...
def load_iwec_data(csv_path, sim_year, sim_month, sim_day):
    ...
    # ------------------------------------------------------------------
    weather_day.index = weather_day.index.floor('h')

    if sim_year is not None and not weather_day.empty:
        try:
            tz = weather_day.index.tz
            if tz is not None:
                naive_index = weather_day.index.tz_convert(None)
                remapped = naive_index.map(lambda ts: ts.replace(year=sim_year))
                weather_day.index = remapped.tz_localize(tz)
            else:
                weather_day.index = weather_day.index.map(lambda ts: ts.replace(year=sim_year))
            print(f"[Weather] Remapped fallback records to simulation year {sim_year}.")
        except Exception as e:
            print(f"[Weather] Year remapping failed ({e}); keeping IWEC year labels.")
...
def compute_pet_series(tdb_2m, tr_2m, v_2m, rh_2m,
    ...
    def _pet_to_float(value):
        """Ensure PET results from pythermalcomfort are plain floats."""
        candidate = value
        if hasattr(candidate, "pet"):
            candidate = getattr(candidate, "pet")
        try:
    ...
    for i, (tdb, tr, v, rh) in enumerate(zip(tdb_2m, tr_2m, v_2m, rh_2m)):
        ...
        pet_float = _pet_to_float(pet)

        if i < n_debug:
            print(f"Point {i}:")
            print(f"  Inputs: tdb={tdb:.2f}, tr={tr:.2f}, v={v:.2f} (used: {v_stable:.2f}), rh={rh:.2f}")
            print(f"  Output: PET={pet_float:.2f}")

        pet_values.append(pet_float)
    
    print("----------------------------------------\n")
    return np.array(pet_values)
...
def main_coupled_run(
    ...
    # Optional per-surface physical properties; if rho & cp are provided, C_face derives from rho*cp.
    rad_k_ground=None,
    rad_k_wall=None,
    rad_k_roof=None,
    rad_rho_ground=None,
    ...
    for start, end, name in building_face_ranges:
        ...
        centroid = (
            tri_centers_all[indices].mean(axis=0).tolist()
            if indices.size > 0
            else [0.0, 0.0, 0.0]
        )
    ...
    # Derive volumetric heat capacity from rho*cp if provided; otherwise use rad_C_face_* defaults.
    _C_ground = rad_C_face_ground
    _C_wall = rad_C_face_wall
    _C_roof = rad_C_face_roof
    if rad_rho_ground is not None and rad_cp_ground is not None:
        _C_ground = rad_rho_ground * rad_cp_ground
    if rad_rho_wall is not None and rad_cp_wall is not None:
        _C_wall = rad_rho_wall * rad_cp_wall
    if rad_rho_roof is not None and rad_cp_roof is not None:
        _C_roof = rad_rho_roof * rad_cp_roof

    C_face = np.empty(F, dtype=np.float64)
    C_face[is_ground] = _C_ground
    C_face[is_roof] = _C_roof
    C_face[is_wall] = _C_wall
    C_face[is_other] = _C_wall # Default for other slanted surfaces
    ...
    for t_sgt in sim_times:
        ...
        def _create_plotter(*args, **kwargs):
            """Factory that respects vedo_display_mode."""
        ...
        if run_cfd:
            ...
            if is_first_iteration:
                ...
                vp_phys = _create_plotter(shape=(2, 2), size=(1200, 1000), title="Initial Physical Properties")
        ...
        if run_radiation:
            ...
            # CTF formula: q_in[t] = Y0*T_out[t] + Y1*T_out[t-1] + Phi*q_in[t-1]
            q_in_current = (CTF_Y0 * T_surf_C_this_step + 
                            CTF_Y1 * T_out_prev + 
                            CTF_Phi * q_in_prev)
            ...
            # Calculate PET at 2m above ground (requires CFD slice data)
            slice_data_2m = None
            if run_cfd and slice_sample_data:
        ...
        if run_radiation and pet_values is not None:
            try:
                if np.any(np.isfinite(pet_values)):
                    local_idx = int(np.nanargmax(pet_values))
                    pet_value = float(pet_values[local_idx])
                    if ground_face_indices.size > local_idx:
                        ground_face_idx = ground_face_indices[local_idx]
                    else:
                        ground_face_idx = ground_face_indices[0]
                ...
                print(f"[Metrics] Warning while tracking hotspots: {hotspot_error}")
        ...
        if t_sgt.hour % 3 == 0:
            ...
            if run_radiation:
                ...
                vp_rad = _create_plotter(shape=(1, 4), size=(2000, 500), title=f"Radiation Fields at {time_label}")
            ...
            if run_cfd:
                vp_slices = _create_plotter(shape=(len(slice_heights), 3), size=(1400, 900), title=f"CFD Slice Fields at {time_label}")
                ...
                time_str = t_sgt.strftime('%Y%m%d_%H%M')
                vp_facets = _create_plotter(shape=(2, 3), size=(1800, 1000), title=f"CFD Surface Fields at {time_label}")
                ...
                if run_radiation:
                    ...
                    cooling_load_viz = hourly_Q_cooling_W.copy() / 1000.0 # Convert to kW
                    ...
                    if 'max_load_for_viz' not in locals():
                        ...
                        max_load_for_viz = 200 * mean_area / 1000.0 # Scale max_load for kW
                    
                    vmin, vmax = 0, max(max_load_for_viz, 0.1) # Ensure vmax is at least 0.1 kW
                    
                    vp_facets.show(_cmap_mesh(mesh_all, cooling_load_viz, title="Hourly Cooling Load (kW)", cmap_name='jet', vmin=vmin, vmax=vmax, filename=os.path.join(vtk_dir, f"cooling_load_hourly_{time_str}.vtk")), at=5, title="Hourly Cooling Load (kW)")
                else:
    ...
    # --- Display Final Cumulative Cooling Load ---
    if run_radiation:
        print("\n[PHASE 2.5] Displaying final cumulative results.")
        vp_cumulative_energy = _create_plotter(shape=(1, 1), size=(800, 700), title="Cumulative Energy Analysis")
        ...
        annotation = Text2D(
            "Final cumulative cooling load (kWh) across façades/roofs\nClose this window to finish the simulation.",
            pos='bottom-left',
            s=0.8,
            c='white',
        )
        vp_cumulative_energy.show(annotation, at=0)
        #vp_cumulative_energy.screenshot(os.path.join(screenshot_dir, "final_cumulative_cooling_load.png"))
...
if __name__ == "__main__":
    ...
    vedo_display_mode_input = 'non-interactive' # 'interactive', 'non-interactive', or 'off'
    # --- END OF USER INPUT ---
